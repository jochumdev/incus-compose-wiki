...
leafwiki_updated_at: "2026-07-05T04:06:36.353767823Z"
...
See [Builds](/docs/v1/builds) for supported options, builder selection, and platform handling.

### Health Checks

Supported via the `ic-healthd` sidecar. See [Health Checking](/docs/v1/healthd) for full details,
...
Any Incus instance config key is accepted. See [Architecture](/docs/v1/architecture#x-incus-raw-incus-options) for full details.
...
`restart` without a healthcheck — see [Health Checking](/docs/v1/healthd#restart-without-a-test).
...
See [healthd.md](/docs/v1/healthd#dockerfile-healthcheck-not-supported) for the background.
...
> See [Getting Started](/docs/v1/getting-started#incus-must-listen-on-the-network-required).
...
explicitly — see [Network Configuration](/docs/v1/healthd#network-configuration).
